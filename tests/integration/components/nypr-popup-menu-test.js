...
moduleForComponent('nypr-popup-menu', 'Integration | Component | nypr popup menu', {
  integration: true
});
...
test('it renders', function(assert) {
  this.render(hbs`
    ...
    {{/nypr-popup-menu}}
  `);
  assert.equal(this.$('.nypr-popupmenu-button').text().trim(), 'Text', 'button text should render');
...
test('it toggles the popup when you click the button', function(assert) {
  assert.expect(3);
  this.render(hbs`{{nypr-popup-menu}}`);
...
test('it closes the popup when you click outside', function(assert) {
  assert.expect(1);
  this.render(hbs`{{nypr-popup-menu}} <div id="outside">outside</div>`);
  this.$('.nypr-popupmenu-button')[0].click();
...
test('it closes the popup when you send a close action', function(assert) {
  assert.expect(1);
  this.render(hbs`{{#nypr-popup-menu as |close|}} <button id="close" {{action close}}>close</button> {{/nypr-popup-menu}}`);
  this.$('.nypr-popupmenu-button')[0].click();
